...
- which output the side pane shows, and the document open in it (`TaskDetailsContext`, selection state from `agent/model.ts`; v3.0 — the side pane replaced v2.0's rows that expanded in place, which had replaced the `agent-artifacts-panel` right split), opened from the outputs bar, the title-bar toggle, tool rows, a finding's Evidence link, the palette or ⌘I (v3.1: ⌘I toggles the pane, opening on the first output the Task has);
...
`frontend/src/components/Composer.tsx` is the only Agent input. It is a docked, bordered card with an accent focus ring (v3.0): paperclip attach + textarea + model chip (`ModelChip`, backed by `/model-providers`; switching activates a provider server-side; *Set up a model…* when none is configured; v3.1: **Runtime offline** with a danger dot, not clickable, when `/model-providers` cannot be reached — `data-offline`) + Delegate / Steer / Stop. Usage (`ContextMeter`) lives in the model menu, not on the Composer bar. Shortcuts exist; they are not painted as a persistent legend on the input. Attachments are keyed by task id. While busy, a present file labels the primary action Delegate (queued Direction), never Steer.
...
`frontend/src/components/TaskDetails.tsx` exports `TaskDetails` — the outputs bar under the Result (one button per available output, with its count) — and `TaskInspector`, the side pane on the right of the document. The pane has one tab per available output; the title-bar toggle, tool rows, a finding's Evidence link and the palette open it on the matching output; ⌘I toggles it (v3.1; `toggleAgentArtifacts()`, opening on the first output the Task has); the close button and Esc close it; a selection with nothing behind it settles on the output the pane shows, so the Report loads (v3.1); focus moves into it when it opens. It is resizable by dragging its left edge (352–880px; double-click resets; the width is kept per device in `localStorage` under `saw.sidepaneWidth`) and below 1100px it overlays the document. v2.0's rows that expanded in place and the v1.11–v1.19 `agent-artifacts-panel` right split are retired. Outputs:

```ts
"evidence" | "report" | "execution"
```

- **Evidence** — persisted evidence/finding/activity truth. Since v3.1 it reads the same findings list as the Result (`taskFindings()` → `lib/findings.ts unifyFindings`: the recorded conclusion's findings joined by `session_findings` via the provenance projection, deduplicated on their words, most severe first; a conclusion finding the work also recorded takes that record's id). Its count equals that list plus attached files. Order: Findings (each expands to detail + source — in the conclusion / recorded while working · confidence · tool · time · gap) → Current understanding → Attached evidence.
- **Reports** — the Task report (`GET /sessions/{id}/report?lang=`, the UI language passed by `api/tasks.ts getTaskReport(id, lang)`).
...
### 6.x Outputs made real (v3.1.0)

No migration; the runtime, the security floor and the single submit path are unchanged.

- **Task report.** `sessions/session_report.render_session_report(..., lang=)` behind `GET /sessions/{session_id}/report?lang=en|zh` (optional; English by default; anything starting `zh` selects Chinese). Order: title + one meta line (Directions · tool calls · when) → Goal (session goal, else the first Direction) → Conclusion (latest recorded `record_conclusion` answer; without one a note plus the latest answer excerpt; with no Work Result *No Work Result yet*) → Findings (one list: the conclusion's, then memory findings recorded while working, then the analyses', deduplicated on text, most severe first, severity words localized) → Next steps (the conclusion's) → Investigation / 工作过程 (per Direction) → Coverage and gaps (grounded in · not verified · left open · limitations, from the tool trace and memory) → What the Agent established → Tools run → Analyses → Attached evidence → Error triage → Rule-derived suggestions → Usage → Audit trail → Safety (always). `_section()` writes nothing for an empty body. Only the module's own words are localized (`_COPY`); the Agent's words are reproduced as recorded. Redacted (`redact_text`), bounded, one-line-sanitized; no raw rows, secrets or chain-of-thought. Pinned by `sidecar/tests/test_v310_report.py`.
- **One findings model.** `frontend/src/lib/findings.ts` `unifyFindings(conclusion, recorded, provenance)` feeds both `TaskResult` (via `TaskDocument`) and the Evidence tab (`TaskDetails.taskFindings`). Each finding keeps its provenance chain when one was recorded; `TaskResult` ends the row with an **Evidence** link (hover previews the source call, click opens the side pane on that finding) or *No direct evidence*. The separate `ProvenanceMark` list under the figures is gone. A Result without a recorded conclusion shows the answer, then the recorded findings.
- **⌘I.** Toggles the side pane (`toggleAgentArtifacts()`); `TaskInspector` settles a selection with nothing behind it on the output it shows.
- **Truthful chrome.** `ModelChip` reads *Runtime offline* (danger dot, not clickable, `data-offline`) when the provider list cannot be fetched.
- **Design system finished.** No `*.tsx` carries a raw colour or type utility (`components/v310.test.tsx`); surface stylesheets live in `frontend/src/styles/`; the `--text-xs` / `--text-base` / `--text-lg` aliases (and Tailwind `xs` / `base` / `lg`) are removed — five sizes, five names (`--text-2xs` · `--text-sm` · `--text-prose` · `--text-xl` · `--text-2xl`); `.ui-scrim` is the one scrim (it fades as a sibling of the opaque sheet); status reads as a dot or badge beside neutral text in settings test results, the cloud tester, S3 error cards, call detail and tool results.
- **Figures.** Light-theme `--viz-2 #e05a26`, `--viz-3 #11906a`, `--viz-4 #ad7d00`, `--viz-5 #d24e86` (same hues, stepped darker) so every series clears 3:1 on white and panel surfaces; inventory ranked bars align to the top of their column.
- **Palette.** The unused v1.16 `prefill` palette action is removed.

### 6.x Design system v3 (v3.0.0)

Frontend only; no Sidecar, API, security or migration change.

- **Tokens.** Five type sizes (11 · 13 · 15 · 20 · 28; in v3.0 older names were aliases — v3.1 removed them, see below); a cool-neutral ladder with every text step AA on `--hover`; one accent (`--accent`, `--accent-soft`, `--accent-dim`, `--accent-fg`, `--accent-text`) for the primary action, selection, focus, links and live progress; status apart; `--space-1…12` on a 4px grid; radii 6 / 10 / 14; `--shadow-elev` / `--shadow-pop`; `--duration-fast` 120 · `--duration-base` 200 · `--duration-slow` 280ms. See `design-tokens.md`.
...
- one findings list for the Result and Evidence, the report language, ⌘I toggling, *Runtime offline*, and no raw colour or type utility in a component (v3.1, `components/v310.test.tsx`).
