...
resolved pyproject's loose ranges at install time: v0.55.0 was developed against
openai-agents 0.17.8 and validated by CI against 0.19.4. `requirements.lock`
(generated by `scripts/lock-sidecar-deps.py`) pins the 64-package runtime closure
that the suite is green against, and every CI/release install uses `-c`. The
ranges express what the code supports; the lock records what was verified. A test
asserts the three SDK facts v0.55.0's tool gating depends on — `get_all_tools` is
called inside the run loop, it honours `is_enabled`, and `FunctionTool` is not
frozen — so a future bump that breaks one fails loudly instead of silently
un-gating the tools.

Two SDK capabilities that had never been used are now set. `timeout_seconds`
bounds every tool (120s, 900s for the bucket-walking ones) with
`error_as_result`, so an endpoint that accepts a connection and goes silent —
a routine finding for this product — surfaces as evidence rather than holding the
turn open. `temperature` is pinned at 0.2 (not 0: strict-greedy decoding makes
some models loop on a failing call instead of changing approach), so two runs of
one investigation differ because of the bucket, not the sampler.
