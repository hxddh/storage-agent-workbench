...
Findings and key figures are clickable when a provenance chain exists (`GET /agent-tasks/{id}/provenance`). Hover shows tool, time, and coverage; click opens Evidence in the side pane and anchors to that finding. A missing chain reads **No direct evidence chain** — never a fabricated source.
...
v3.0.0 is design system v3 on the native shell (v1.17 made it the Codex window). Visual language is specified in
