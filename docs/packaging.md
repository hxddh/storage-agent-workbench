...
> **Storage Agent v1.19.0 packaging contract.** Unchanged from v0.96.0 in topology; v1.10 registers the `storage-agent://` deep-link scheme in `tauri.conf.json` (`plugins.deep-link`) and adds a native menu bar, notifications and a global summon shortcut in `src-tauri/src/lib.rs` — no new bundle resources.
>
