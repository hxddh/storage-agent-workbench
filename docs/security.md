...
A continuation (`kind=resume` / `kind=retry`, v2.2) stores the user's Direction as written. The model's copy alone carries a continuation note and a digest of calls already completed (`task_runtime/continuation.py`): one line per call (tool, target, one-line result) taken from the stopped execution's durable `tool.completed` events — sanitized and bounded when they were written — across at most 3 links of its chain, capped at 24 lines / 2 400 characters (oldest dropped first) and redacted again. No raw rows, tool outputs or chain-of-thought enter the prompt through it.
