...
> live progress (status stays a separate palette); one component library
> (`components/ui.tsx`). Durable outputs (Evidence · Report · Execution) open in
> a resizable, closable **side pane** on the right instead of expanding in
> place; the empty start has three starters that only fill the Composer;
> figures are cards with a Chart/Table toggle and a validated categorical
> palette; the palette is Recent · Actions. No migration (head stays **031**).
> **v2.2.0 was Native agent depth** (`docs/releases/2.2.0.md`): every tool is
...
> moves the window (detail rows became the side pane in v3.0). No migration (head stays **031**).
...
> shapes.
> **v1.17.0 is the Codex window.** Title bar is name + state; Find and the
> palette are keyboard; ContextMeter lives in the model menu; the empty start
> is greeting + Composer with no glyph; the user bubble is a quiet fill;
> approval is sentence-case hairline; *Worked for {t}* is wall-clock only;
> attachments are per-task; copy is Direction / Execution / Work Result.
> No migration (head stays **030**).
> **v1.18.0 is the native core.** No HTTP route starts a run or moves data
> without a Decision; revisits run on the Sidecar's own clock; a Steer is a
> `steer` turn item; `AgentTask` is the one composition root and the `api/`
> adapters speak Task. No migration (head stays **030**).
