...
/** A proposed next step. One click hands the task back to the agent (it does it
 * inline) or opens the purpose-built dialog — no configuration form. */
export function ProposalCard({
  ...
  return (
    <button
      ...
      className="group/prop flex w-full animate-fade-in-up items-center gap-2.5 rounded-xl border border-accent/25 bg-accent-dim/40 px-3 py-2.5 text-left transition-colors hover:border-accent/45 hover:bg-accent-dim/70"
    >
      ...
      <span className="shrink-0 rounded-md bg-accent/15 px-2.5 py-1 text-[12px] font-medium text-accent-soft transition-colors group-hover/prop:bg-accent/25">
        {t("proposal.run")}
      </span>
    </button>
  );
