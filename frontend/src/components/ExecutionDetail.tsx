...
import type { TaskRecord, TaskFinding, TaskMessage } from "../types";
import {
  applyCompacted,
  applyDelta,
  applyPlan,
...
function ExecutionDocument({
  ...
  const [document, setDocument] = useState<TaskRecord | null>(null);
  ...
  useEffect(() => {
    ...
    const readDocument = () => getTaskRecord(taskId)
  ...
  const workResult = useMemo<TaskMessage | null>(() => {
  ...
  const findings = useMemo<TaskFinding[]>(() => {
  ...
  return (
    <article className="native-execution-doc" data-testid="execution-detail-body" data-execution-id={executionId}>
      ...
      <section className="native-execution-doc-block" data-testid="execution-steps">
        {hasRows ? (
          <TranscriptItems items={turn.items} live={running} taskId={taskId} startedAt={startedMs} />
        ) : (
...
export type ExecutionDetailProps = ComponentProps<typeof ExecutionDocument>;

/**
 * Review boundary for one durable Execution (`task_executions` + its
 * `execution_events`): the same durable log the transcript reads, never the
