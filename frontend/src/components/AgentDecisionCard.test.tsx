...
const action = (requiresConfirmation: boolean): NextAction => ({
...
describe("Agent decision boundary", () => {
  it("renders an ordinary next action without inventing approval", () => {
    render(<I18nProvider><AgentNextAction action={action(false)} onRun={vi.fn()} /></I18nProvider>);
  ...
  it("elevates backend-required confirmation into an explicit decision", () => {
    const onRun = vi.fn();
    render(<I18nProvider><AgentNextAction action={action(true)} onRun={onRun} /></I18nProvider>);
