...
const en: Dict = {
  ...
  "attach.pickTypeHint": "Choose a type for the attached file (access log or inventory) before delegating.",
  "attach.promptInventory": "Analyze this inventory file.",
  "attach.promptLog": "Analyze this log file.",
  "grounding.title": "Result grounding",
  ...
  "msg.edit": "Revise direction",
  "msg.regenerate": "Run this direction again",
  ...
  "sugg.checkup": "Storage checkup",
  "sugg.cost": "Cost review",
  "sugg.drift": "Drift check",
  "sugg.account": "Map account & buckets",
  "sugg.config": "Storage checkup",
  "sugg.optimize": "Cost review",
  ...
  "prompt.checkup": "Run a read-only storage checkup of my bucket: configuration, lifecycle, encryption, and public-access posture. Capture a bounded baseline of the findings. Do not invent numbers that evidence does not support.",
  "prompt.cost": "Run a cost and lifecycle review using the local simulator on this Task's bounded inventory aggregates. If inventory is missing or the price table is still the example schedule, report those as gaps — do not invent dollar figures. Draft a remediation plan from the simulation. Storage Agent stays read-only; I will apply any lifecycle JSON in my own console.",
  "prompt.drift": "Compare this Task's latest bounded evidence against the stored baseline and produce a Drift report. Classify findings as added, resolved, or still present. If there is no baseline, say so explicitly — do not fabricate a trend.",
  "prompt.config": "Run a read-only storage checkup of my bucket: configuration, lifecycle, encryption, and public-access posture. Capture a bounded baseline of the findings. Do not invent numbers that evidence does not support.",
  "prompt.account": "Discover my account and map out all my buckets, regions, and their configuration.",
  "prompt.optimize": "Run a cost and lifecycle review using the local simulator on this Task's bounded inventory aggregates. If inventory is missing or the price table is still the example schedule, report those as gaps — do not invent dollar figures. Draft a remediation plan from the simulation. Storage Agent stays read-only; I will apply any lifecycle JSON in my own console.",
  // Agent execution states
  "card.agentName": "Storage Agent",
  "think.0": "Thinking…",
  "think.1": "Consulting StorageOps skills…",
  "think.2": "Grounding in evidence…",
  "think.3": "Synthesizing Work Result…",
  ...
  "settings.priceTitle": "Storage price table",
  "settings.priceHint": "Used only by the local cost simulator. This is ordinary configuration, not a secret, and must never contain credentials. Dollar figures stay gaps until you confirm the table against your bill.",
  "settings.priceExample": "Example prices — calibrate against your bill before treating dollar figures as estimates.",
  "settings.priceGbMonth": "Storage (USD / GB-month)",
  "settings.priceClass": "Storage class",
  "settings.priceConfirm": "I have calibrated these rates against my bill",
  "settings.priceSave": "Save price table",
  "settings.priceSaved": "Saved",
  // providers
  ...
  "wizard.step1Body": "An LLM API key so the Agent can interpret evidence and produce Work Results.",
  ...
  "memory.rolled": "The Agent replays the most recent {shown} of {total} task history; earlier steps are no longer in its active context — it continues from this memory and the summary instead.",
  // screen-reader announcements
  "a11y.task": "Task",
  "a11y.working": "The Agent is working.",
  "a11y.resultReady": "Work Result ready.",
  "a11y.turnFailed": "Agent execution failed.",
  "a11y.needKey": "A model provider is required before the Agent can execute this task.",
  // token detail
  "metrics.cachedHint":
  ...
  // execution budget + in-execution dedupe
  "metrics.ofBudget": "of budget",
  "metrics.budgetHint":
    "This execution used {pct}% of its {n}-token task budget. The budget is this Agent's per-execution ceiling, not a provider limit.",
  "metrics.repeatsHint":
    "{n} identical tool call(s) were reused from earlier in this execution instead of being re-run — the payload was not paid for twice.",
};
...
const zh: Dict = {
  ...
  "attach.pickType": "分析为：",
  "attach.pickTypeHint": "委派前请为附件选择类型（访问日志或清单）。",
  "attach.promptInventory": "请分析这个清单文件。",
  "attach.promptLog": "请分析这个日志文件。",
  "grounding.title": "结果依据",
  ...
  "trace.kinds": "{n} 种工具",
  "trace.failed": "{n} 项失败",
  "msg.edit": "修订 Direction",
  "msg.regenerate": "按同一 Direction 再执行",
  "msg.branch": "从这里分出一个新的 Task",
  "msg.showMore": "展开",
  "msg.showLess": "收起",
  "sugg.diagnose": "诊断错误",
  "sugg.logs": "分析访问日志",
  ...
  "prompt.checkup": "请对这个桶做一次只读存储体检：配置、生命周期、加密和公共访问态势。把有界发现固化为基线。证据不足时明确标为缺口，不要编造数字。",
  "prompt.cost": "请用本机模拟器，基于这个 Task 已有的有界清单聚合做成本与生命周期审查。如果没有清单，或价格表仍是示例价，请把这些标为缺口，不要编造金额。根据模拟结果起草一份修复方案。Storage Agent 保持只读；我会在自己的控制台应用生命周期 JSON。",
  "prompt.drift": "请把这个 Task 最新的有界证据与已存储的基线对比，产出 Drift 报告。将 findings 分为新增、已解决、仍存在。如果没有基线，请明确说明，不要虚构趋势。",
  "prompt.config": "请对这个桶做一次只读存储体检：配置、生命周期、加密和公共访问态势。把有界发现固化为基线。证据不足时明确标为缺口，不要编造数字。",
  "prompt.account": "请梳理我的账号，列出所有桶、所在区域及其配置。",
  "prompt.optimize": "请用本机模拟器，基于这个 Task 已有的有界清单聚合做成本与生命周期审查。如果没有清单，或价格表仍是示例价，请把这些标为缺口，不要编造金额。根据模拟结果起草一份修复方案。Storage Agent 保持只读；我会在自己的控制台应用生命周期 JSON。",
  "card.agentName": "云存储 Agent",
  "think.0": "思考中…",
  "think.1": "正在参考 StorageOps 专业方法…",
  "think.2": "正在以证据为依据…",
  "think.3": "正在生成 Work Result…",
  "think.working": "正在执行…",
  "run.queued": "排队中",
  "run.running": "运行中",
  "run.done": "完成",
  ...
  "settings.priceTitle": "存储价格表",
  "settings.priceHint": "仅供本机成本模拟器使用。这是普通配置，不是密钥，也绝不能包含凭据。在你按账单校准并确认之前，金额会保持为缺口。",
  "settings.priceExample": "示例价格 — 请按你的账单校准后再把金额当作可执行的估算。",
  "settings.priceGbMonth": "存储（USD / GB-月）",
  "settings.priceClass": "存储类别",
  "settings.priceConfirm": "我已按账单校准这些费率",
  "settings.priceSave": "保存价格表",
  "settings.priceSaved": "已保存",
  "prov.title": "提供商",
  ...
  "wizard.welcomeBody": "一个本地优先的云存储 Agent，面向 S3 兼容系统——覆盖运维、分析与管理：诊断、访问日志与清单分析、账号与桶配置审查、错误排查、用量与成本画像、优化建议。一切都在本机运行；密钥仅保存在本机加密保险库中。",
  "wizard.step1Title": "添加模型提供商",
  "wizard.step1Body": "一个大模型 API Key，让 Agent 能够解读证据并生成 Work Result。",
  "wizard.step2Title": "添加云存储提供商",
  "wizard.step2Body": "只读的 S3 凭证，用于对桶进行实时检查与分析。",
  ...
  "memory.rolled": "Agent 只回放最近 {shown} 条（共 {total} 条）Task 历史；更早的 Steps 已不在当前上下文中——Agent 会基于这份 Memory 与摘要继续执行。",
  "a11y.task": "Task",
  "a11y.working": "Agent 正在工作。",
  "a11y.resultReady": "Work Result 已就绪。",
  "a11y.turnFailed": "Agent 执行失败。",
  "a11y.needKey": "需要先配置模型提供商，Agent 才能执行当前任务。",
  "metrics.cachedHint": "其中 {n}（{pct}%）输入 token 命中了端点的 prompt 缓存——缓存输入通常便宜得多。",
  "metrics.reasoningHint": "输出中有 {n} 个 token 用于推理：计费，但不作为 Work Result 正文显示。",
  ...
  "metrics.ofBudget": "预算",
  "metrics.budgetHint": "本次执行用掉了 {n} token 任务预算的 {pct}%。该预算是 Agent 当前执行的任务上限，并非服务商限制。",
