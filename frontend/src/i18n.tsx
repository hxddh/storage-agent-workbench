...
const en: Dict = {
  ...
  "settings.autonomyHint":
    "How much the in-chat agent does on its own. Read-only investigation always runs automatically; this controls whether the agent also executes read-only runs (diagnostics, config review, account discovery) or only proposes them. Data-moving work always needs your confirmation.",
  "settings.autonomyAdvisory": "Advisory",
  "settings.autonomyAssisted": "Assisted",
  "settings.autonomyAutonomous": "Autonomous (read-only)",
  "settings.autonomyAdvisoryHint": "Only proposes — runs nothing itself.",
  "settings.autonomyAssistedHint": "Runs read-only diagnostics/reviews itself; confirms the rest.",
  "settings.autonomyAutonomousHint": "Chains read-only runs freely; confirms data-moving work.",
  "settings.safetyTitle": "Safety",
  "settings.safety":
    "Secrets are stored only in the OS keychain — never in the database, logs, reports, or model prompts. Cloud access is read-only and the agent has no write or destructive capability. Data-moving work (downloads, large scans, dataset analysis) always requires your confirmation regardless of the autonomy setting.",
  // providers view
  ...
  "wizard.welcomeBody":
    "A local-first agent for object storage on S3-compatible systems — operations, analytics, and management. Diagnose issues, analyze logs and inventory, review configuration, profile usage and cost, and optimize. Everything runs on your machine; secrets stay in the OS keychain.",
...
const zh: Dict = {
  ...
  "settings.autonomy": "智能体自主度",
  "settings.autonomyHint":
    "对话中的智能体可以自行做多少事。只读调查始终自动执行；此项控制智能体是否还能自行执行只读运行（诊断、配置审查、账号梳理），还是仅给出建议。涉及搬运数据的操作始终需要你确认。",
  "settings.autonomyAdvisory": "仅建议",
  "settings.autonomyAssisted": "协助",
  "settings.autonomyAutonomous": "自主（只读）",
  "settings.autonomyAdvisoryHint": "只提建议——自己不执行任何运行。",
  "settings.autonomyAssistedHint": "自行执行只读诊断/审查；其余需确认。",
  "settings.autonomyAutonomousHint": "自由串联只读运行；搬运数据的操作需确认。",
  "settings.safetyTitle": "安全",
  "settings.safety":
    "密钥仅保存在操作系统钥匙串中——绝不会写入数据库、日志、报告或模型提示词。云访问为只读，智能体没有任何写入或破坏性能力。无论自主度如何设置，涉及搬运数据的操作（下载、大规模扫描、数据集分析）始终需要你确认。",
  "prov.title": "提供商",
  ...
  "wizard.welcomeBody": "一个本地优先的云存储 Agent，面向 S3 兼容系统——覆盖运维、运营分析与管理:诊断、访问日志与清单分析、账号与桶配置审查、错误排查、用量与成本画像、优化建议。一切都在本机运行;密钥仅保存在操作系统钥匙串中。",
