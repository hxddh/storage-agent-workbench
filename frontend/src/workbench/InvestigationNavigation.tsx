...
function InvestigationRow({
  ...
  return (
    ...
    >
      ...
      <div className="mt-1 flex min-w-0 items-center gap-2 pl-3.5 text-2xs leading-4 text-gray-500">
    ...
    >
      ...
      <div className="group/brand flex items-center gap-2.5 px-3.5 pb-2 pt-3.5">
        ...
        <div className="min-w-0">
          ...
          <div className="text-2xs uppercase tracking-[0.12em] text-gray-500">{copy.investigations}</div>
        </div>
        <button
          type="button"
          onClick={onToggleCollapse}
      ...
      <div className="px-2.5 pb-1">
        ...
        >
          ...
          <kbd className="rounded border border-edge bg-elevated/70 px-1.5 py-px text-2xs tracking-wide text-gray-500 opacity-0 transition-opacity group-hover:opacity-100">⌘N</kbd>
        </button>
      </div>
      ...
      <nav className="min-h-0 flex-1 overflow-auto px-1.5 pb-2" aria-label={copy.investigations}>
        ...
        {pinned.length ? (
          <section aria-label={copy.pinned}>
            <div className="px-2 pb-1 pt-2 text-2xs font-medium uppercase tracking-[0.11em] text-gray-500">{copy.pinned}</div>
            {pinned.map(row)}
          </section>
        ) : null}
        ...
        {DAY_BUCKETS.map((bucket) => {
          ...
          return (
            <section key={bucket} aria-label={t(`rail.day.${bucket}`)}>
              <div className="px-2 pb-1 pt-2 text-2xs font-medium uppercase tracking-[0.11em] text-gray-500">{t(`rail.day.${bucket}`)}</div>
              {rows.map(row)}
            </section>
          );
        })}
        ...
        {archived.length ? (
          <section aria-label={copy.archived}>
            <button
              ...
              className="mt-2 flex w-full items-center gap-1.5 px-2 py-1 text-2xs font-medium uppercase tracking-[0.11em] text-gray-500 transition-colors hover:text-gray-400"
            >
