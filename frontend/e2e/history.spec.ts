...
/**
 * A multi-turn Task must keep its history readable.
 *
 * The rest of the E2E covers the shell and providers, and the
 * smoke spec sends exactly ONE Direction. Nothing ever produced a long
 * Task and then checked what remained on screen, which is why the suite
 * stayed green while the released app reportedly showed no history.
 *
 ...
 *  - a seeded session (`seed.ts`), for the Work Results that the offline
 *    path can never produce.
 */

const composer = (page: Page) => page.getByTestId("agent-composer").getByRole("textbox");
/** The Task document itself — never the task list, which repeats titles. */
...
test.describe("a multi-turn task", () => {
  test("keeps the FIRST exchange visible after a second one", async ({ page }) => {
    ...
    await expect(task(page).getByText(/AccessDenied/).first()).toBeVisible();
  });
  ...
  test("history survives a reload", async ({ page }) => {
    ...
    await expect(task(page).getByText(/AccessDenied/).first()).toBeVisible({ timeout: 15_000 });
  });
  ...
  test("cards appear in the order they happened", async ({ page }) => {
    ...
    expect(order.first, "AccessDenied must be present in the Task").toBeGreaterThanOrEqual(0);
    expect(order.second, "NoSuchBucket must be present in the Task").toBeGreaterThanOrEqual(0);
...
test.describe("a long conversation", () => {
  test("shows every exchange, oldest first", async ({ page }) => {
    ...
    const txt = (await task(page).evaluate((el) => el.textContent ?? "")).replace(/\s+/g, " ");
    for (let i = 0; i < 12; i++) {
      expect(txt, `exchange ${i} must be in the Task`).toContain(`QUESTION-${String(i).padStart(2, "0")}`);
    }
  ...
  test("an old answer is still the whole answer, not a one-line summary", async ({ page }) => {
    ...
    // default. Measured on a seeded 40-turn session at 1440x900: the Task's
    ...
    // through your own Task showed rows and whitespace, which is what
  ...
  test("a user Direction keeps copy and does not offer branch chrome", async ({ page }) => {
    ...
    const last = task(page).getByText("QUESTION-11").last();
