...
v0.55.0 introduced the write lock (``db.transaction(conn)`` since v0.78.0)
because parallel tool calls share ONE connection, and a connection has ONE
transaction: thread A's ``commit()`` commits B's half-written work, and B's own
``commit()`` then raises ``cannot commit - no transaction is active``. The agent
sees a FAILED call for work that actually succeeded, and rule 17's "every tool
call is recorded" quietly does not hold.
...
def _run_forced_pair(conn, rounds: int) -> tuple[int, list[str]]:
    ...
    ``note_fact`` writes agent memory under db.transaction(conn).
    ``list_uploaded_files`` writes an audit row and commits. Both are agent tools the model can call in
