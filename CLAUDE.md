...
> **Implementation contract for Storage Agent v1.17.3.**
>
...
The v1.17.3 product invariant is:
...
- (v1.12) **compaction** (`agent_runtime/compaction.py`): when the last turn's reported input usage reaches 60 % of the model's context window, the runtime runs one tool-less, bounded, redacted summary step before the model loop, stores it on the typed context (`summary_sanitized` / `summary_through_seq`, migration 030), appends `context.compacted`, and the prompt replays only later messages with the summary in its stable half — `summary` and `agent_memory` are omitted (the summary replaces them); `storage_task_context` stays. Uncompacted turns send writable `agent_memory` plus a deterministic summary only for uncopied facts; assistant replay is a digest. `RunConfig.group_id` is the task id (openai-agents prompt-cache routing). `POST /agent-tasks/{id}/compact` runs it on demand for an idle task. Never a second Agent; never raw rows; never chain-of-thought;
...
- (v1.17.3) Title bar is name + state only (⌘F / ⌘K stay; Search is the quieter sidebar row). Engine tools (`simulate_storage_cost` and the other storage engines) are gated as `storage_engines`, not CORE. Tool descriptions are one sentence plus Args. A first tool delivery over 6 000 characters is digested before the model sees it (`read_skill` exempt). Consumed outputs compact after one later step. The tool-output floor is 48k characters (~12k tokens), 12 % of the window. No migration.
